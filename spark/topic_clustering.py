...
def spit(filePath, data, overwrite=False):
    ...
    with codecs.open(filePath, mode, 'utf-8') as x:
        x.write(data)
...
def doc_word_counts(sw, doc_tuple):
    ...
    filtered_words = [
        word for line in line_words
        # flatten
        for word in line if not word_filtered(word)
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--stopwords", default="etc/english.stopwords", help="stop words file")
    parser.add_argument(
        "--vocab_index", default="tmp/vocab.idx", help="index of vocab")
    parser.add_argument(
        "output_path", help="output directory for topic clustering")
    ...
    output = doc_wcs.map(to_vector).map(
        lambda x: "{}\t{}".format(x[0], " ".join(map(str, x[1]))))
